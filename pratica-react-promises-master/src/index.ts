...
function testCompact() {
  ...
  const resultadoCorreto = expected.every(
    (item, index) => item === resultado[index],
  );
  ...
  console.assert(
    resultadoCorreto,
    "todos os objetos esperados devem estar presentes",
  );
}
...
function testFromPairs() {
  ...
  // verificação do método.
  const correto = Object.keys(esperado).every(
    (key) => esperado[key] === resultado[key],
  );
...
async function testFila() {
  ...
  console.assert(
    todasMensagensEscritas,
    "todas as mensagens devem ser escritas",
  );
  for (const menssagem of mensagens) {
    const encontrada = await consumirDaFila();
    console.log(encontrada);
    console.assert(
      encontrada === menssagem,
